...
@njit
def _MI1(rx, tx1, Powers0, Powers1, X0, X1, phase, P_noise, N_fec_cells):
    N_cells = len(rx)
    N_codewords = N_cells // N_fec_cells
    m_rx = rx.reshape(-1, N_fec_cells)
    ...
    for i in range(N_codewords):
        ...
        for k in range(N_fec_cells):
            ...
            den = -np.inf
            for x0 in X0:
                D = m_rx[i,k] - C_sqrt*x0 - I_sqrt*m_tx1[i,k]*m_phase[i,k]
                num = _max_star(num, -np.abs(D)**2/P_noise)
                for x1 in X1:
                    D = m_rx[i,k] - C_sqrt*x0 - I_sqrt*x1*m_phase[i,k]
                    den = _max_star(den, -np.abs(D)**2/P_noise)
...
@njit
def _MI1_0(rx, tx0, tx1, Powers0, Powers1, X0, X1, phase, P_noise, N_fec_cells):
    N_cells = len(rx)
    N_codewords = N_cells // N_fec_cells
    m_rx = rx.reshape(-1, N_fec_cells)
    m_tx0 = tx0.reshape(-1, N_fec_cells)
    ...
    for i in range(N_codewords):
        ...
        for k in range(N_fec_cells):
            ...
            D = m_rx[i,k] - C_sqrt*m_tx0[i,k] - I_sqrt*m_tx1[i,k]*m_phase[i,k]
...
@njit
def _MI0_1(rx, tx0, tx1, Powers0, Powers1, X0, X1, phase, P_noise, N_fec_cells):
    N_cells = len(rx)
    N_codewords = N_cells // N_fec_cells
    m_rx = rx.reshape(-1, N_fec_cells)
    m_tx0 = tx0.reshape(-1, N_fec_cells)
