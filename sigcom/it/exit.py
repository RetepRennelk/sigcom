...
from sigcom.it.util import getMutualInfo, getNoisePower
...
def VN_exit_function(H, Ias, P_noise):
    edge_stats = degree_distribution_edge_view(H)
    MIs = np.zeros(len(Ias))
    for k, Ia in enumerate(Ias):
        MI = 0
        for stats in edge_stats.vn:
            dv = stats[0]
            prob = stats[1]
            MI_tmp = getMutualInfo(4/P_noise + (dv-1)*getNoisePower([Ia]))[0]
            MI += prob * MI_tmp
        MIs[k] = MI
    return MIs
...
def VN_exit_function_demapper(H, Ias, codebits, demap_callback):
    MIs = np.zeros(len(Ias))
    N = H.shape[1]
    K = N-H.shape[0]
    for k, Ia in enumerate(Ias):
...
def CN_exit_function(H, Ias):
    ...
    for k, Ia in enumerate(Ias):
        ...
        for stats in edge_stats.cn:
            dc = stats[0]
            prob = stats[1]
            MI_tmp = 1-getMutualInfo((dc-1)*getNoisePower(1-Ia))
            MI += prob*MI_tmp
        MIs[k] = MI
    return MIs
